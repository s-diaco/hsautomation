...
class BandLeader:
    ...
    def currently_playing(self):
        ...
        try:
            if self.is_playing():
                ...
                album_detail = self.browser.find_element(
                    By.CSS_SELECTOR, ".detail-album > a"
                )
                album_title = album_detail.text
                album_url = album_detail.get_attribute("href").split("?")[0]
                artist_detail = self.browser.find_element(
                    By.CSS_SELECTOR, ".detail-artist > a"
                )
    ...
    def play(self, track=None):
        ...
        elif isinstance(track, int) and track <= len(self.track_list) and track >= 1:
